...
async def handle_updates(updates: list):
    ...
    for update in updates:
        try:
            log.info(f"🔄 پردازش update_id: {update.get('update_id')}")
            msg = update.get("message", {})
            text = msg.get("text", "").strip()
            chat_id = str(msg.get("chat", {}).get("id", ""))

            if not text or not chat_id:
                log.info(f"⏭ پیام بدون متن یا chat_id — رد شد")
                continue

            log.info(f"📩 پیام از chat_id={chat_id}: '{text[:60]}'")
...
async def poll_telegram_updates(offset: int = 0) -> tuple[list, int]:
    ...
    async with httpx.AsyncClient(timeout=40) as client:
        try:
            ...
            # اگه conflict بود (ربات جای دیگه هم داره اجرا میشه)
            if not data.get("ok"):
                err_code = data.get("error_code", 0)
                desc = data.get("description", "")
                log.error(f"❌ Telegram API error: {err_code} — {desc}")
                if err_code == 409:
                    log.warning("⚠️ Conflict! شاید چند نسخه ربات دارند اجرا میشن")
                await asyncio.sleep(5)
                return [], offset

            updates = data.get("result", [])
            if updates:
                log.info(f"📩 {len(updates)} پیام جدید دریافت شد")
                offset = updates[-1]["update_id"] + 1
            return updates, offset

        except httpx.ReadTimeout:
            # timeout عادیه — long polling
            return [], offset
        except Exception as e:
            log.error(f"❌ خطا در poll: {type(e).__name__}: {e}")
            await asyncio.sleep(5)
            return [], offset
...
async def telegram_listener():
    ...
    # اول offset رو به آخرین آپدیت برسون تا پیام‌های قدیمی پردازش نشن
    offset = 0
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if results:
                ...
                log.info(f"⏭ offset شروع: {offset}")
    except Exception as e:
        log.warning(f"⚠️ نتوانست offset اولیه را بگیرد: {e}")
    ...
    while True:
        try:
            ...
            log.error(f"❌ خطا در listener loop: {e}")
            await asyncio.sleep(5)
