...
async def send_telegram_message(text: str, chat_id: str = None) -> bool:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for i, chunk in enumerate(chunks, 1):
            header = f"📄 [{i}/{total}]\n" if total > 1 else ""
            resp = await client.post(url, json={"chat_id": target, "text": header + chunk})
            resp.raise_for_status()
        ...
        try:
            ...
            if text in ["/start", "start"]:
                ...
                await send_telegram_message(
                    ...
                    f"📋 دستورات:\n"
                    f"  /report — گزارش جدید (موضوع تصادفی)\n"
                    f"  /topic — گزارش با موضوع دلخواه\n"
                    f"  /level — تغییر سطح (عمومی/تخصصی)\n"
                    f"  /schedule — تنظیم زمان‌بندی شخصی\n"
                    f"  /history — گزارش‌های قبلی\n"
                    f"  /help — راهنما\n\n"
                ...
                await send_telegram_message(
                    ...
                    "🔹 /report — گزارش جدید با موضوع تصادفی\n"
                    "🔹 /topic — گزارش با موضوع دلخواهت\n"
                    "🔹 /level — تغییر سطح محتوا\n"
                    "   public = عمومی (با مثال‌های روزمره)\n"
                    "   expert = تخصصی (با جزئیات آماری)\n"
                    "🔹 /schedule — تنظیم فاصله گزارش‌ها\n"
                    "🔹 /history — ۱۰ گزارش آخر\n\n"
...
async def main():
    ...
    await send_telegram_message(
        ...
        f"⚡ گزارش فوری: رمز {REPORT_PASSWORD}\n"
        f"📝 موضوع دلخواه: /topic\n"
        f"🎚 تغییر سطح: /level\n"
        f"⏰ زمان‌بندی شخصی: /schedule"
    )
