        ...
        try:
            ...
            if text == "/start":
                await send_telegram_message(
                    ...
                    f"هر روز ساعت {SEND_HOUR}:00 یک گزارش علمی برات می‌فرستم.\n\n"
                    f"برای گرفتن گزارش همین الان، رمز مخصوص رو بفرست.",
                    chat_id=chat_id
                )

            # رمز مخفی برای گزارش فوری
            elif text == REPORT_PASSWORD:
                await send_telegram_message("⏳ در حال آماده‌سازی گزارش علمی...", chat_id=chat_id)
                await daily_job(chat_id=chat_id)
...
# وظیفه اصلی روزانه
# ─────────────────────────────────────────
async def daily_job(chat_id: str = None):
