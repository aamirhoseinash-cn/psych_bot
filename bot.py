    ...
    async with httpx.AsyncClient(timeout=90) as client:
        for model in MODELS:
            try:
                ...
                log.info(f"✅ persona محتوا تولید شد")
                return content
            except httpx.HTTPStatusError as e:
                code = e.response.status_code
                err = f"{model}: {code}"
                errors.append(err)
                log.warning(f"⚠️ {err}")
                await asyncio.sleep(5 if code == 503 else 3)
            except Exception as e:
        ...
        try:
            ...
            if persona_key and is_authorized:
                ...
                await send_msg(
                    ...
                    f"{p['tagline']}\n\n"
                    f"موضوعات پیشنهادی:\n{topics_list}\n\n"
                    f"یک موضوع بنویس یا /persona_random بزن برای موضوع تصادفی:",
                    chat_id=chat_id
                )
            ...
            if state == "waiting_persona_topic":
                ...
                lang = user.get("language", "fa")
                topic_input = None if text in ["/persona_random", "تصادفی"] else text
                await send_persona_report(chat_id, persona_key, topic_input, lang)
                continue
            ...
            if text == "/personas":
                await send_msg(
                    "🧑‍💼 کدام روانشناس را انتخاب می‌کنی؟\n\n"
                    "🛋 فروید — ناخودآگاه، رؤیا، سرکوب\n"
                    "🌑 یونگ — کهن‌الگو، سایه، فردیت‌یابی\n"
                    "🕯 یالوم — مرگ، آزادی، معنا\n"
                    "🕊 فرانکل — رنج، معنا، امید\n\n"
                    "روی یکی بزن:",
                    chat_id=chat_id,
                    show_menu=False
                )
                # نمایش کیبورد شخصیت‌ها
            ...
            elif text == "/persona_random" and user.get("selected_persona"):
                persona_key = user.get("selected_persona", "jung")
                lang = user.get("language", "fa")
