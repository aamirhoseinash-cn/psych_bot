...
async def send_daily_concept_to_all():
    """ارسال مفهوم روز به همه کاربران فعال"""
    log.info("🌅 ارسال مفهوم روز...")
    ...
    data = load_data()
    users = [u for u in data.get("users", {}).values() if u.get("authorized") and u.get("active")]
    
    log.info(f"📤 ارسال مفهوم روز به {len(users)} کاربر")
    for user in users:
        try:
...
async def main():
    ...
    # مفهوم روز — هر روز ۷:۳۰ صبح (نیم ساعت قبل از گزارش اصلی)
