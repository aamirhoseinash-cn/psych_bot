...
# مدل‌ها به ترتیب اولویت — 2.5 Pro قوی‌ترین رایگان
MODELS = [
    "gemini-2.5-pro",        # قوی‌ترین — ۱۰۰ درخواست/روز
    "gemini-2.5-flash",      # جایگزین اول — ۲۵۰ درخواست/روز
    "gemini-2.5-flash-lite", # جایگزین دوم — ۱۰۰۰ درخواست/روز
]
...
def build_prompt(topic: str, level: str, language: str, source_pack: str) -> str:
    ...
    # ── قانون زبان ──
    if language == "fa":
        lang_rule = """LANGUAGE — NON-NEGOTIABLE:
Write the ENTIRE report in Persian (Farsi) only. Every sentence must be in Persian.
Technical terms: write Persian term + English equivalent in parentheses. Example: ذهن‌آگاهی (Mindfulness)
References list: keep author names and journal names exactly as they appear in SOURCE_PACK (do not translate them to Persian). Only translate the reference format label: write "منابع" as header."""
        word_range = "400 تا 700 کلمه"

    elif language == "en":
        lang_rule = """LANGUAGE — NON-NEGOTIABLE:
Write the ENTIRE report in English only. Every sentence must be in English.
References list: keep exactly as in SOURCE_PACK."""
        word_range = "400 to 700 words"

    else:
        lang_rule = """LANGUAGE — NON-NEGOTIABLE:
Write each section in Persian first, then English immediately after.
Persian comes first and must be complete. English follows as a full parallel.
References: keep author names and journal names in original language."""
        word_range = "600 تا 900 کلمه"

    # ── تفاوت واقعی دو سطح ──
    if level == "expert":
        level_desc = """CONTENT LEVEL: EXPERT (for psychologists, researchers, graduate students)

Your job: Explain the scientific status of this topic as a knowledgeable colleague would.
— Discuss the theoretical framework and its evolution
— Explain what specific studies found and HOW they found it (methodology matters)
— Discuss contradictions between studies and what they mean theoretically
— Identify the key unresolved questions in the field
— Use precise terminology (explain briefly if rare)
— Do NOT oversimplify, but do NOT pile on statistics — one key finding per paragraph
— Tone: collegial, precise, intellectually honest"""

    else:
        level_desc = """CONTENT LEVEL: GENERAL PUBLIC (for curious adults with no psychology background)

Your job: Be a wise, knowledgeable friend who explains something genuinely fascinating.
— Open with something that makes the reader stop and think: a surprising fact, a paradox, or a relatable moment
— Use concrete everyday scenarios to illuminate abstract concepts
  GOOD: "تصور کنید همیشه از فلان خصلت در دیگران آزرده می‌شوید — علم نشان می‌دهد این آزردگی اغلب آینه‌ای از خود شماست"
  BAD: "این مفهوم در زندگی روزمره کاربرد دارد"
— Every concept needs a specific, memorable example from daily life (work, relationships, decisions, habits)
— Teach something actionable: what can the reader DO differently today because of this?
— Reveal one thing that most people believe but science shows is wrong
— Tone: warm, curious, intelligent — never condescending, never superficial"""
...
1. Use ONLY sources from SOURCE_PACK. If SOURCE_PACK has real papers, every claim must link to them.
2. If no SOURCE_PACK: describe findings accurately without fabricating any citation. Write "طبق تحقیقات موجود" instead.
3. NEVER invent a paper title, author name, year, or journal — readers will search for these.
4. References section: list only real sources from SOURCE_PACK. Format:
   — If source is English: keep author names, journal name, and year in original. Example: Beck, A.T. (1979). Cognitive therapy of depression. Guilford Press.
   — If source is Persian: write in Persian.
5. NO emojis inside the body text. Only allowed: section headers (maximum 1 emoji per header).
6. NO therapy advice, diagnosis, or medication recommendations.
7. NO exaggerated or motivational language.
8. Length: {word_range}. Be substantial but not padded.
