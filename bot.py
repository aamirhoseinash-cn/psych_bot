...
async def send_persona_report(chat_id: str, persona_key: str, topic, language: str) -> None:
    ...
    try:
        content = await generate_persona_content(persona_key, chosen_topic, language)
        report_id = datetime.now().strftime("%m%d%H%M")
...
# ─────────────────────────────────────────
async def main():
    log.info(f"🤖 ربات v6.0 | ساعت پیش‌فرض: {SEND_HOUR}:00")
    ...
    await send_msg(
        f"🧠 ربات روانشناسی علمی v6.0 فعال شد!\n\n"
