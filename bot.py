...
MODELS = [
    "deepseek/deepseek-r1-0528:free",      # قوی‌ترین — استدلال علمی عالی
    "meta-llama/llama-4-maverick:free",    # جایگزین اول
    "google/gemini-2.0-flash-exp:free",    # جایگزین دوم
    "meta-llama/llama-4-scout:free",       # جایگزین سوم
]
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for model in MODELS:
            try:
                ...
                log.warning(f"⚠️ مدل {model} خطا داد: {e} — مدل بعدی...")
                await asyncio.sleep(2)

    raise RuntimeError("همه مدل‌ها خطا دادند. لطفاً API Key را بررسی کنید.")
