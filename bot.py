...
async def send_read_button(chat_id: str, report_id: str, topic: str):
    """ارسال دکمه تأیید خواندن گزارش"""
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    short_topic = topic[:30] + "..." if len(topic) > 30 else topic
    async with httpx.AsyncClient(timeout=10) as client:
        await client.post(url, json={
            "chat_id": chat_id,
            "text": "آیا این گزارش را خواندید؟",
            "reply_markup": {
                "inline_keyboard": [[
                    {"text": "✅ خوندم و مفید بود", "callback_data": f"read:{report_id}"},
                    {"text": "⏭ بعداً می‌خونم", "callback_data": f"skip:{report_id}"},
                ]]
            }
        })
...
def update_user_interests(data: dict, chat_id: str, topic: str):
    """استخراج تگ‌های علاقه‌مندی از موضوع و ذخیره در پروفایل"""
    if chat_id not in data["users"]:
        return
    
    # ذخیره تاریخچه موضوع با تاریخ
    ...
    # فقط ۳۰ موضوع آخر
    data["users"][chat_id]["topic_history"] = history[-30:]
    
    # استخراج تگ‌های ساده از موضوع
    tag_map = {
        "اضطراب": "anxiety", "افسردگی": "depression", "تروما": "trauma",
        "PTSD": "trauma", "دلبستگی": "attachment", "رابطه": "relationships",
        "کودک": "child_dev", "حافظه": "memory", "خواب": "sleep",
        "تصمیم": "decision", "انگیزه": "motivation", "شخصیت": "personality",
        "CBT": "cbt", "ذهن‌آگاهی": "mindfulness", "مغز": "neuroscience",
        "یادگیری": "learning", "هیجان": "emotions", "استرس": "stress",
        "اعتیاد": "addiction", "خودشناسی": "self_awareness",
    }
    
    tags = data["users"][chat_id].get("interest_tags", [])
    topic_lower = topic.lower()
    for keyword, tag in tag_map.items():
        if keyword.lower() in topic_lower and tag not in tags:
            tags.append(tag)
    
    data["users"][chat_id]["interest_tags"] = tags[-20:]


async def get_smart_suggestion(chat_id: str) -> str | None:
    """
    بر اساس تاریخچه کاربر، یه پیشنهاد هوشمند تولید می‌کند.
    فقط اگه کاربر حداقل ۳ موضوع داشته باشد.
    """
    data = load_data()
    user = data.get("users", {}).get(chat_id, {})
    history = user.get("topic_history", [])
    
    if len(history) < 3:
        return None
    
    # آخرین موضوع و تگ‌های علاقه
    last_topic = history[-1]["topic"] if history else ""
    tags = user.get("interest_tags", [])
    recent_topics = [h["topic"] for h in history[-5:]]
    
    if not last_topic:
        return None
    
    prompt = f"""A psychology bot user has read these topics recently:
{chr(10).join(f"- {t}" for t in recent_topics)}

Their interest tags: {", ".join(tags) if tags else "general"}

Generate ONE short suggestion in Persian (max 2 sentences) recommending a related topic they haven't explored yet.
Format: "هفته پیش راجع به [موضوع قبلی] خوندی — [پیشنهاد موضوع جدید مرتبط] رو هم دوست داری؟"
Be specific and relevant. No generic suggestions."""
                ...
                async with httpx.AsyncClient(timeout=10) as client:
                    await client.post(
                        ...
                        json={
                            ...
                            "text": "✅ ثبت شد! علاقه‌مندی‌هات به‌روز شد.",
                        }
                    )
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                await client.post(
                    ...
                    json={
                        "chat_id": chat_id,
                        "message_id": message_id,
                        "text": "⏭ باشه! هر وقت خوندی می‌تونی دوباره از /history پیداش کنی.",
                    }
                )
    except Exception as e:
