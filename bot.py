...
از DeepSeek API استفاده می‌کند — 5 میلیون توکن رایگان، بدون کارت بانکی
"""
...
# مدل اصلی + جایگزین
MODELS = [
    "deepseek-chat",    # DeepSeek V3 — سریع و قوی
    "deepseek-reasoner", # DeepSeek R1 — استدلال عمیق‌تر
]
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        for model in MODELS:
            try:
                ...
                err_msg = f"مدل {model} خطا: {e.response.status_code} — {body[:300]}"
                log.warning(f"⚠️ {err_msg}")
                errors.append(err_msg)
                await asyncio.sleep(3)
            except Exception as e:
                err_msg = f"مدل {model} خطا: {type(e).__name__}: {e}"
                log.warning(f"⚠️ {err_msg}")
                errors.append(err_msg)
                await asyncio.sleep(3)
